<script>
    ...
    const pendingOrders = $derived(page.props.pending_orders ?? []);
    ...
    const stats = $derived([
        { label: 'Order Perlu Review', value: String(kpis.pending_count ?? 0), sub: 'menunggu persetujuan', icon: 'cart', tone: 'accent' },
        { label: 'Transaksi Bulan Ini', value: String(kpis.orders_month ?? 0), sub: `${kpis.orders_approved ?? 0} selesai`, icon: 'history', tone: 'primary' },
...
<AppLayout>
    <div>
        ...
        <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
            <DashboardCard title="Order Perlu Review" sub="Proyeksi margin per order" class="lg:col-span-2">
                ...
                {#if pendingOrders.length === 0}
                    <p class="py-10 text-center text-sm text-slate-400">Tidak ada order menunggu persetujuan.</p>
                {/if}
            </DashboardCard>
        ...
        <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
            <DashboardCard title="Transaksi Terbaru" sub="8 transaksi terakhir" class="lg:col-span-3">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="text-left text-xs uppercase tracking-wide text-slate-400">
